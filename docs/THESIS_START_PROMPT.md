...
### Step 0: 現状深掘りフェーズ（ChatGPT徹底質問・自己報告）

汚染防止のため、必ず「新しいCursorのチャット」で実施してください。

- ChatGPTに「ChatGPTへの自己報告依頼テンプレ」を投げ、以下を一次出典付きで収集：
  1) 最新の7章構成（目的・主要トピック・想定アウトカム）
  2) 指導教官提言（優先順位・該当メッセージ参照）
  3) 必ず引用候補トップ20（DOI/URL付き）
